...
if (isset($_POST['form_key']) && $_POST['form_key'] == $_SESSION['form_key']) {
	...
	try {
		...
		sendMessageToTelegramBot('Có Đơn - Bàn Là Hơi Nước Cầm Tay - ' . implode(' | ', $data));
	} catch(Exception $e) {
		sendMessageToTelegramBot('Có Đơn - Ghi vào Gsheet lỗi ' . $e->getCode() . ' - Bàn Là Hơi Nước Cầm Tay - ' . implode(' | ', $data));
	}
